...
foreach ($results1 as $array) {
  foreach ($array as $key) {
    $filename = "tasks" . $key . ".php";
...
<pre class="mb-5 text-lg font-sans">
<?php echo $title ?>
</pre>
    <div class="exercise bg-gray-200 w-full h-48 rounded relative mb-7 pt-3">
      <pre class="pl-4">
    ...
    <button class="bg-red-600 p-3 px-5 rounded-full text-white font-bold" onclick="checkAnswer()"
      ...
      ответ</button>
    <a class="bg-red-600 p-3 px-5 rounded-full text-white font-bold hidden  nextButton" <?php echo "href=\"tasks{$arrayId[$index + 1]}.php\""; ?>
      id="nextButton">Следующий</a>
  </main>
  <!-- <footer></footer> -->
