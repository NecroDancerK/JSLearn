...
if ($isAdmin == 1) { ?>
  ...
  <body>
    ...
    <main class="pt-40 pb-10 mx-auto w-3/5" id="click">
      <h2 class="text-5xl mb-5">Изменение упражнения:</h2>
      <form class="flex flex-col gap-4 bg-gray-200 p-7 text-xl rounded" action="tasksEdit.php" method="post">
        <label>
          Номер задания (Пойдет в название упражнения):
          <input class="border border-red-600 rounded" type="number" name="taskNumber" min="1"
            value="<?php echo $task_number ?>">
        </label>
        <label>
          Тема задания:
        ...
        <label class="flex flex-col">
          ...
          <textarea class="border border-red-600 mt-4 resize-none h-28 rounded"
            name="title"><?php echo $title ?></textarea>
        </label>
        <label class="mb-7 flex flex-col">
          ...
          <textarea class="border border-red-600 mt-4 resize-none h-28 rounded" name="task"><?php echo $task ?></textarea>
        </label>
